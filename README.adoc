...
The above command returns a value of 20, indicating that a total of 20 requests are made to the `system` service that are multiplied by the following:

* 5 Istio requests that included the initial request and 4 retry requests that you enabled in the [hotspot=attempts file=0]`traffic.yaml` file
* 4 requests sent by MicroProfile that included the initial request and [hotspot=mpRetry file=1]`3` retry requests
...
The above command returns 5, indicating that a total of 5 requests are made to the `system` service.
...
The above command returns 1, indicating that totally 1 request is made to the `system` service.
The Istio retries that you enabled on the `inventory` service are not performed.
Although the [hotspot=getPropertiesForHost file=1]`getPropertiesForHost()` method failed, the [hotspot=fallbackMethod file=1]`getPropertiesFallback()` fallback method is called.
